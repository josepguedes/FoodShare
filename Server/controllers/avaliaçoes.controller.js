  ...
  try {
    ...
    if (idAvaliado) {
      ...
      if (!avaliado) {
        throw new ErrorHandler(
          404,
          `Utilizador com ID ${idAvaliado} não encontrado`
        );
      }
    ...
    try {
      avaliacoes.rows.forEach((avaliacao) => {
        avaliacao.links = [
          {
            rel: "self",
            href: `/avaliacoes/${avaliacao.IdAvaliacao}`,
            method: "GET",
          },
          {
            rel: "delete",
      ...
      links: [
        ...
        ...(page > 1
          ? [
              {
                rel: "pagina-anterior",
                href: `/avaliacoes?limit=${limit}&page=${page - 1}`,
                method: "GET",
              },
            ]
          : []),
        ...(avaliacoes.count > page * limit
          ? [
              {
                rel: "proxima-pagina",
                href: `/avaliacoes?limit=${limit}&page=${+page + 1}`,
...
const createAvaliacao = async (req, res, next) => {
  try {
    const { IdAnuncio, IdAutor, IdAvaliado, Comentario, Classificacao } =
      req.body;
    ...
    if (req.user.IdUtilizador != IdAutor) {
      return res.status(403).json({
        message:
          "Não tem permissão para criar avaliações em nome de outro utilizador",
      });
    }
    ...
    if (
      !Number.isInteger(Number(Classificacao)) ||
      Classificacao < 1 ||
      Classificacao > 5
    ) {
      throw new ErrorHandler(
    ...
    if (Comentario.length <= 1 || Comentario.length > 255) {
      throw new ErrorHandler(
        400,
        "Comentário deve ter no máximo 255 caracteres"
      );
    }
    ...
    if (avaliacaoExistente) {
      throw new ErrorHandler(
        409,
        "Já existe uma avaliação sua para este anúncio"
      );
    }
    ...
    const result = await Avaliacao.findOne({
      ...
      attributes: [
        [
          db.sequelize.fn("AVG", db.sequelize.col("Classificacao")),
          "mediaClassificacao",
        ],
        [
          db.sequelize.fn("COUNT", db.sequelize.col("IdAvaliacao")),
          "totalAvaliacoes",
        ],
      ],
    });
    ...
    if (result) {
      const mediaClassificacao = parseFloat(
        result.getDataValue("mediaClassificacao")
      ).toFixed(1);

      // Update user's rating
      await db.Utilizador.update(
        { Classificacao: mediaClassificacao },
        { where: { IdUtilizador: IdAvaliado } }
      );
      ...
      return res.status(201).json({
        ...
        data: {
          ...
          totalAvaliacoes: result.getDataValue("totalAvaliacoes"),
        },
      });
    }
    ...
    if (err.name === "SequelizeForeignKeyConstraintError") {
      return next(
        new ErrorHandler(
          400,
          "Referência inválida: anúncio ou utilizador não existe"
        )
      );
    }
...
const updateAvaliacao = async (req, res, next) => {
  try {
    ...
    if (req.user.IdUtilizador != avaliacao.IdAutor) {
      return res.status(403).json({
        message:
          "Não tem permissão para editar avaliações de outros utilizadores",
      });
    }
    ...
    next(
      new ErrorHandler(500, "Erro interno do servidor ao atualizar avaliação")
    );
  }
};
...
const deleteAvaliacao = async (req, res, next) => {
  try {
    ...
    if (!req.user.IsAdmin && req.user.IdUtilizador != avaliacao.IdAutor) {
      return res.status(403).json({
        message:
          "Apenas o autor da avaliação ou um administrador pode eliminá-la",
      });
    }
    ...
    if (err.name === "SequelizeForeignKeyConstraintError") {
      return next(
        new ErrorHandler(
          400,
          "Erro: A avaliação não pode ser eliminada devido a dependências"
        )
      );
    }
    ...
    next(
      new ErrorHandler(500, "Erro interno do servidor ao eliminar avaliação")
    );
  }
};
